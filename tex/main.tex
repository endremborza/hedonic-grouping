...
\begin{abstract}
We study a strongly generalized stable matching problem --- the \emph{grouping problem} --- in which agents have strict preferences over arbitrary subsets of the population. This framework subsumes both the Stable Marriage Problem and the Stable Roommates Problem, neither of which permits agents to reason over group compositions of arbitrary size or structure. We introduce \emph{group stability} as the appropriate solution concept: a grouping is stable if no subset of agents unanimously prefers forming a coalition exclusively among themselves over their current assignment. This notion coincides with core stability in the hedonic games literature. We present a recursive two-phase algorithm that either returns a group-stable partition or certifies that none exists, and prove that it always halts on finite preference profiles. We further establish that under standard structural restrictions on preferences, the algorithm reduces to the Gale-Shapley algorithm for the stable marriage problem and to Irving's algorithm for the stable roommates problem, demonstrating that it is a genuine unifying framework for stable matching.
\end{abstract}

\begin{keyword}
stable matching \sep hedonic games \sep coalition formation \sep core stability \sep grouping problem \sep recursive algorithm
\end{keyword}
...
Grouping might not only be useful in the case of people or institutions. Connections between computers are an indispensable part of our lives. Treating computers as agents who act according to their self-interest, and applying mechanisms to them similarly to humans, is not a new concept \citep{nisan99}. Stable coalition formation among computational agents is equally relevant in distributed systems, peer-to-peer networks, and multi-agent planning.

The remainder of this paper is organized as follows. Section~\ref{sec:related} surveys the hedonic games literature that frames the grouping problem. Section~\ref{sec:framework} defines the formal framework and introduces group stability. Section~\ref{sec:algorithms} presents the three-phase algorithm and proves its correctness and termination. Section~\ref{sec:connections} establishes the formal reductions to the Stable Marriage Problem and the Stable Roommates Problem. Section~\ref{sec:conclusion} concludes.

\section{Related Work}
\label{sec:related}

The grouping problem belongs to the literature on \emph{hedonic games}, a term introduced by \cite{drzezegreenberg80} to describe settings where agents form coalitions and each agent's utility depends solely on the composition of their own coalition. This \emph{hedonic} property --- that agents are indifferent to how others outside their coalition are grouped --- is precisely the assumption maintained throughout this paper.

The foundational paper of \cite{bogomolnaia02} establishes the core existence and algorithmic questions for hedonic coalition formation under strict preferences. They demonstrate that even under strict preferences, a core-stable partition need not exist, and characterize preference domains that guarantee existence. \cite{banerjee01} independently study core stability in coalition formation games and provide sufficient conditions based on the notion of \emph{top coalitions}: if every subset of agents contains a coalition that is unanimously preferred by its members, a core-stable partition exists.

The computational complexity of finding core-stable partitions is generally intractable. \cite{ballester04} establishes NP-completeness results for hedonic games under several preference representations. \cite{woeginger13} surveys known tractability and hardness results, distinguishing between different stability concepts (core, Nash, individual stability) and different preference encodings.

The \emph{group stability} concept defined in this paper coincides with the standard \emph{core} in hedonic games: a partition lies outside the core if and only if a blocking coalition exists --- a set of agents who all prefer the coalition consisting of themselves alone over their current assignment. This equivalence allows the results of this paper to be read directly as an algorithmic contribution to core computation in hedonic games.

Several restricted subclasses of hedonic games have been studied algorithmically. \cite{cechlárová02} study the \emph{stable partition problem}, a hedonic game in which preferences satisfy an additively separable structure; they show that a stable partition always exists in this setting. \cite{alcalde04} study coalition formation in research collaboration and characterize stability under various restrictions. The current paper makes no assumption on the structural form of preferences beyond strict ordering --- it operates on the full preference profile over all subsets --- and is therefore applicable to the most general hedonic game formulation.

What distinguishes this work from the literature is the recursive algorithmic approach, which generalizes both the Gale-Shapley algorithm \citep{galeshapley62} and Irving's algorithm for the stable roommates problem \citep{irving85}. To our knowledge, no prior work presents a single algorithm that subsumes both classical stable matching algorithms as special cases.
...
However, $\alpha_5$ rejecting the proposal of $\alpha_3$ leads to an incorrect conclusion: the grouping that creates $\{\alpha_3 , \alpha_5 \}$ and $\{ \alpha_1 , \alpha_2 , \alpha_4 \}$ is group stable. Moreover, the grouping that includes $\{ \alpha_1 , \alpha_2 , \alpha_5 \}$, whose prospect caused $\alpha_5$ to reject $\alpha_3$, is not stable: $\alpha_1$ would defect from that group to form $\{ \alpha_1 , \alpha_3 \}$, which $\alpha_3$ also prefers.

We therefore refine the rejection criterion. An agent is only permitted to use a held proposal to reject another if the held proposal is \textbf{considerable} in the following sense.
...
\cite{tan91} distinguishes two approaches to stability in the roommate problem: a logical one, and an algorithmic one deducible from \cite{irving85}. We follow the algorithmic approach, extending the reduction algorithms to perform the role of Irving's second phase, so that the algorithm scales down to the roommate problem.
...
We formalize Algorithm~\ref{alg:fun} as a function. Its inputs are a grouping problem already subjected to Simplified Reduction: $A$ the set of agents, $M$ the remaining possible matchings, $\Omega$ the complete preference profile, and $J$ the group from which agents are forced to move on (the \emph{exception}).
...
It is clear then that Algorithm \ref{alg:rec} finds a stable solution if there is one, and reports that there is none, if there is none.\footnote{An implementation in Python 2.7 with test cases is available at \url{http://rajk.eu/borza/grouping/grouping.py}}

\section{Connections to Classical Matching Algorithms}
\label{sec:connections}

We now formalize the claim that Algorithm~\ref{alg:rec} is a genuine unifying framework for stable matching, by showing that it reduces to the two canonical algorithms when standard structural restrictions are imposed on the preference profiles.

\subsection{Reduction to the Stable Marriage Problem}

In the Stable Marriage Problem, the agent set $A$ is partitioned into two disjoint sets $A_M$ and $A_W$ of equal cardinality. Each agent $\alpha_i \in A_M$ has preferences only over singletons drawn from $A_W$, and vice versa. Formally, $M(\alpha_i)$ contains only sets of the form $\{\alpha_j\}$ where $\alpha_j$ lies in the opposite partition.

\begin{mylem}
\label{lem:gs}
When preferences are restricted to a bipartite structure with group size two, Algorithm~\ref{alg:sim} (Simplified Reduction) terminates with a stable matching that is outcome-equivalent to the man-optimal stable matching produced by the Gale-Shapley algorithm.
\end{mylem}

\begin{proof}
Under bipartite size-two restrictions, a proposal from $\alpha_i \in A_M$ to $\{\alpha_j\}$ is \emph{considerable} for $\alpha_j$ if and only if $\alpha_j$ also proposes $\{\alpha_i\}$. The reduction of $M$ upon holding a considerable proposal corresponds exactly to striking out dominated entries in the preference list. Since each $M(\alpha_i)$ contains only singletons from the opposite partition, the loop in Algorithm~\ref{alg:sim} mirrors the propose-hold-reject cycle of Gale-Shapley: each $\alpha_j \in A_W$ holds the best proposal received and rejects all dominated ones. The algorithm terminates when every agent holds exactly one proposal; this is precisely the stable matching identified by Gale-Shapley for a proposer-optimal outcome. Correctness follows from Corollaries~\ref{sta:rej} and~\ref{sta:hold}, which reduce to the classical stability condition in this restricted setting.

Furthermore, the ``moving on'' phase of Algorithm~\ref{alg:rec} is never triggered: in the bipartite equal-cardinality setting, the Simplified Reduction always terminates with all agents holding a proposal, so no agent reaches a state with multiple unresolved proposals that would require forced progression.
\end{proof}

\subsection{Reduction to the Stable Roommates Problem}

In the Stable Roommates Problem, the composition constraint is lifted but the cardinality constraint is retained: $M(\alpha_i)$ contains only singletons drawn from $A \setminus \{\alpha_i\}$, with no bipartite restriction.

\begin{mylem}
\label{lem:irving}
When preferences are restricted to group size two without a bipartite constraint, the combined action of Algorithms~\ref{alg:sim} and~\ref{alg:rec} is outcome-equivalent to Irving's algorithm for the Stable Roommates Problem~\citep{irving85}.
\end{mylem}

\begin{proof}
Phase 1 of Irving's algorithm corresponds directly to Algorithm~\ref{alg:sim}: both execute a propose-hold-reject cycle that terminates with each agent holding at most one proposal, and both eliminate the same set of impossible pairs. Under size-two restrictions, the \emph{considerable} condition again reduces to the requirement that both agents in a proposed pair propose each other.

Phase 2 of Irving's algorithm identifies and eliminates \emph{rotation sequences} (also called preference rings): cyclic structures in the reduced preference table that must be eliminated before a stable matching can be found, if one exists. The ``moving on'' mechanism in Algorithm~\ref{alg:rec} plays the analogous role: forcing $\alpha_i$ to move on from group $J = \{\alpha_i, \alpha_j\}$ corresponds to advancing $\alpha_i$'s proposal past $\alpha_j$ in the table. The return value 1 from Algorithm~\ref{alg:fun} --- triggered when a moved-on agent receives $J$ back as a considerable proposal --- corresponds exactly to the detection of an Irving rotation: the cycle closes, confirming that the omitted pair must form in any stable solution or that no solution exists.

Since both the rotation detection and its consequences (either committing the pair or eliminating all solutions through a return-0 path) are handled identically, Algorithm~\ref{alg:rec} is operationally equivalent to Irving's algorithm on size-two non-bipartite inputs.
\end{proof}

Lemmas~\ref{lem:gs} and~\ref{lem:irving} together establish that Algorithm~\ref{alg:rec} strictly generalizes both classical algorithms. The general grouping problem adds neither a cardinality bound of two nor any compositional constraint, requiring the full recursive apparatus.

\section{Conclusion}
\label{sec:conclusion}

The grouping problem --- a hedonic coalition formation game in which agents hold strict preferences over arbitrary subsets of the population --- is a strong generalization of classical stable matching. We have shown that it admits a unified recursive algorithm that either returns a group-stable (core-stable) partition or certifies that no such partition exists. The algorithm operates in two phases: a Simplified Reduction phase that eliminates impossible groups via a proposal-and-hold mechanism, and a recursive ``moving on'' phase that resolves cyclic deadlocks. Termination is guaranteed by the finiteness of the preference profiles.

The algorithm subsumes the Gale-Shapley algorithm for the Stable Marriage Problem and Irving's algorithm for the Stable Roommates Problem as special cases (Lemmas~\ref{lem:gs} and~\ref{lem:irving}), demonstrating that these classical results are instances of a single underlying mechanism.

Several directions remain open. First, this paper restricts to strict preferences; extending to weak preferences (indifferences) would require adapting the notions of \emph{super-stability} and \emph{weak stability} studied by \cite{irving94sta} and \cite{irving02} to the grouping setting, and would enable the college admissions problem to be treated as a grouping problem. Second, a formal worst-case complexity analysis of Algorithm~\ref{alg:rec} is outstanding; the recursion tree depth is bounded by the total number of possible group memberships, but tight bounds under specific preference structures deserve careful treatment. Third, necessary and sufficient conditions for the existence of a group-stable partition --- analogous to Tan's condition for the roommate problem \citep{tan91} --- could be derived from the structure of the algorithm. Finally, the algorithm could be adapted to optimize over stable solutions, for instance by minimizing maximum regret or satisfying some notion of optimality within the core.


\bibliographystyle{elsarticle-harv}
\bibliography{mechbib}
